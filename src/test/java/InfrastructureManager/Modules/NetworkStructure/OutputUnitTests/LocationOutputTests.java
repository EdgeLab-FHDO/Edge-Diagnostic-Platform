...
public class LocationOutputTests {
	...
	public void deviceLocationverification() throws ModuleExecutionException, JsonProcessingException, InterruptedException {
		...
		String expected = "{\"locationId\":\"location1\",\"deviceRelationId\":\"devLoc1\",\"connectionRelationId\":\"locCon1\",\"latitude\":15.5,\"longitude\":25.5}";
		Assert.assertEquals(expected, sharedLocation.getLocationAsJsonBody());
	}
	
	@SuppressWarnings("deprecation")
	@Test
	public void applicationLocationverification() throws ModuleExecutionException, JsonProcessingException, InterruptedException {
		updateNetwork();
		sharedNetwork.UpdateSharedNetworkObject(network);
	...
	public void incompleteCommandThrowsException() {
		String command = "location device";
