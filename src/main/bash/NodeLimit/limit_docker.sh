  ...
  do
      ...
      then
        ...
        do
          ...
          cgcreate -g cpu:/"$node_tag" #Create a cpu limited group with the name of the node
          cgset -r cpu.cfs_period_us="$period_us" "$node_tag"
          cgset -r cpu.cfs_quota_us="$quota_us" "$node_tag"
      ...
      then
        ...
        echo "$aux" | xargs -t cgclassify -g cpu:/"$read_node_tag"
      fi
